...
// Generated Mar 30, 2016 10:31:43 PM by Hibernate Tools 4.3.1
...
public class CityHome {
	...
	public CityHome(SessionFactory sessionFactory) {
	...
	public List<City> findAllCity() throws Exception {
		...
		try {
			...
			List<City> results = session.createCriteria(City.class).list();
			tx.commit();
	...
	public City findById(java.lang.Integer id) {
		...
		try {
			City instance = (City) sessionFactory.getCurrentSession().get("com.home.model.City", id);
			if (instance == null) {
	...
	public List<City> findByExample(City instance) {
		log.debug("finding City instance by example");
		try {
			List<City> results = (List<City>) sessionFactory.getCurrentSession().createCriteria(City.class).add(create(instance)).list();
			log.debug("find by example successful, result size: " + results.size());
			return results;
		} catch (RuntimeException re) {
			log.error("find by example failed", re);
